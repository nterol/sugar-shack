...
export const createTRPCContext = async (_opts: CreateNextContextOptions) => {
  ...
  const { session } = await createSessionContext(_opts);
...
const cartSessionMiddleware = middleware(async (opts) => {
  ...
  try {
    const verified = <jwt.WithCartSession>(
      jwt.verify(session.user, process.env.JWT_SECRET as string)
    );
    return opts.next({ ctx: { withCartID: verified.cartID } });
  } catch (err) {
    console.log("⚠️ JWT is INVALID");
    session.destroy();
    return opts.next({ ctx: { withCartID: null } });
  }
});
...
export const withCartSessionProcedure = publicProcedure.use(
